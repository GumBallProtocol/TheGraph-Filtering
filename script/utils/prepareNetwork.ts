...
export async function prepare(network: string) {
  try {
    ...
    const templateFilePath = path.join(
      __dirname + "/../../config/" + network + "/" + TEMPLATE_FILE_NAME
    );
    //const constantsOutputPath = path.join(__dirname + '/../../src/utils/' + CONSTANTS_FILE_NAME)
    const templateFileOutputPath = path.join(
      __dirname + "/../../" + TEMPLATE_FILE_NAME
    );
